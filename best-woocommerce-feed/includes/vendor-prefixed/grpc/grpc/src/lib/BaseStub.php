...
class BaseStub
{
    ...
    public function __construct($hostname, $opts, $channel = null)
    {
        ...
        if ($channel) {
            if (!is_a($channel, 'Grpc\Channel') &&
                !is_a($channel, 'RexFeed\Vendor\Grpc\Internal\InterceptorChannel')) {
    ...
    private static function updateOpts($opts) {
        ...
        if (defined('\Grpc\VERSION')) {
            $version_str = \RexFeed\Vendor\Grpc\VERSION;
        } else {
    ...
    {
        if ($new_state == \RexFeed\Vendor\Grpc\CHANNEL_READY) {
            return true;
        }
        if ($new_state == \RexFeed\Vendor\Grpc\CHANNEL_FATAL_FAILURE) {
    ...
    {
        if (is_a($channel, 'RexFeed\Vendor\Grpc\Internal\InterceptorChannel')) {
            return function ($method,
                             $argument,
                             $deserialize,
                             array $metadata = [],
