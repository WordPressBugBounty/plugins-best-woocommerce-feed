...
class GcpExtensionChannel
{
    ...
    public function getConnectivityState($try_to_connect = false)
    {
        ...
        foreach ($this->channel_refs as $channel_ref) {
            ...
            switch ($state) {
                case \RexFeed\Vendor\Grpc\CHANNEL_READY:
                    $ready += 1;
                    break 2;
                case \RexFeed\Vendor\Grpc\CHANNEL_FATAL_FAILURE:
                    $shutdown += 1;
                    break;
                case \RexFeed\Vendor\Grpc\CHANNEL_CONNECTING:
                    $connecting += 1;
                    break;
                case \RexFeed\Vendor\Grpc\CHANNEL_TRANSIENT_FAILURE:
                    $transient_failure += 1;
                    break;
                case \RexFeed\Vendor\Grpc\CHANNEL_IDLE:
                    $idle += 1;
                    break;
            }
        }
        if ($ready > 0) {
            return \RexFeed\Vendor\Grpc\CHANNEL_READY;
        } elseif ($idle > 0) {
            return \RexFeed\Vendor\Grpc\CHANNEL_IDLE;
        } elseif ($connecting > 0) {
            return \RexFeed\Vendor\Grpc\CHANNEL_CONNECTING;
        } elseif ($transient_failure > 0) {
            return \RexFeed\Vendor\Grpc\CHANNEL_TRANSIENT_FAILURE;
        } elseif ($shutdown > 0) {
            return \RexFeed\Vendor\Grpc\CHANNEL_SHUTDOWN;
        }
    }
