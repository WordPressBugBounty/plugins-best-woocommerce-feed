...
use RexFeed\Vendor\GuzzleHttp\Cookie\CookieJar;
use RexFeed\Vendor\GuzzleHttp\Cookie\CookieJarInterface;
use RexFeed\Vendor\GuzzleHttp\Exception\GuzzleException;
use RexFeed\Vendor\GuzzleHttp\Exception\InvalidArgumentException;
use RexFeed\Vendor\GuzzleHttp\Handler\CurlShareHandleState;
use RexFeed\Vendor\GuzzleHttp\Handler\CurlVersion;
use RexFeed\Vendor\GuzzleHttp\Promise as P;
use RexFeed\Vendor\GuzzleHttp\Promise\PromiseInterface;
use RexFeed\Vendor\Psr\Http\Message\RequestInterface;
use RexFeed\Vendor\Psr\Http\Message\ResponseInterface;
use RexFeed\Vendor\Psr\Http\Message\StreamInterface;
use RexFeed\Vendor\Psr\Http\Message\UriInterface;

/**
 * @final
 */
class Client implements ClientInterface, \RexFeed\Vendor\Psr\Http\Client\ClientInterface
{
    use ClientTrait;
     ...
     *   RexFeed\Vendor\GuzzleHttp\Promise\PromiseInterface that is fulfilled with a
    ...
    {
        rexfeed_vendor_trigger_deprecation('guzzlehttp/guzzle', '7.1', '%s::%s() is deprecated and will be removed in 8.0.', __CLASS__, __FUNCTION__);
    ...
    public function requestAsync(string $method, $uri = '', array $options = []): PromiseInterface
    {
        ...
        if ($method !== $normalizedMethod) {
            rexfeed_vendor_trigger_deprecation(
                'guzzlehttp/guzzle',
                '7.11',
    ...
    private function transfer(RequestInterface $request, array $options): PromiseInterface
    {
        ...
        if ('' === $protocolVersion) {
            rexfeed_vendor_trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Sending a request with an empty protocol version is deprecated; guzzlehttp/guzzle 8.0 will reject empty protocol versions.');

            $request = Psr7\Utils::modifyRequest($request, ['version' => '1.1']);
        } elseif (!self::isProtocolVersionValid($protocolVersion)) {
            rexfeed_vendor_trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Sending a request with a malformed protocol version is deprecated; guzzlehttp/guzzle 8.0 will reject malformed protocol versions.');
        }
    ...
    private static function createBodyStream($body): StreamInterface
    {
        ...
        if (\is_scalar($body)) {
            rexfeed_vendor_trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing a non-string scalar to the "body" request option is deprecated; guzzlehttp/guzzle 8.0 will reject non-string scalar bodies.');
    ...
    private static function normalizeNonFiniteFloats(array $values, string $option): array
    {
        foreach ($values as $key => $value) {
            if (\is_array($value)) {
                ...
                rexfeed_vendor_trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing a non-finite float in the "%s" request option is deprecated; guzzlehttp/guzzle 8.0 will reject non-finite floats.', $option);
    ...
    {
        if ('' === $version) {
            rexfeed_vendor_trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing an empty "version" request option is deprecated; guzzlehttp/guzzle 8.0 will reject empty protocol versions.');
