 ...
 */
namespace RexFeed\Google\Http;

use RexFeed\Google\Auth\HttpHandler\HttpHandlerFactory;
use RexFeed\Google\Client;
use RexFeed\Google\Task\Runner;
use RexFeed\Google\Service\Exception as GoogleServiceException;
use RexFeed\GuzzleHttp\ClientInterface;
use RexFeed\GuzzleHttp\Exception\RequestException;
...
class REST
{
    ...
    {
        $runner = new Runner($config, \sprintf('%s %s', $request->getMethod(), (string) $request->getUri()), array(self::class, 'doExecute'), array($client, $request, $expectedClass));
        if (null !== $retryMap) {
    ...
    public static function doExecute(ClientInterface $client, RequestInterface $request, $expectedClass = null)
    {
        try {
            ...
            if ($response instanceof \RexFeed\GuzzleHttp\Message\ResponseInterface) {
