...
if( !defined( 'WATCHDOG_SCHEDULE_HOOK' ) ) {
    define( 'WATCHDOG_SCHEDULE_HOOK', 'rex_feed_watchdog_stuck_feeds' );
}
...
function rex_product_feed_admin_notice() {
	...
	$activate = is_string( $activate ) ? sanitize_text_field( $activate ) : '';
...
if ( is_admin() ) {
	...
	require_once plugin_dir_path( __FILE__ ) . 'admin/class-pfm-feed-nudge.php';
	new PFM_Feed_Nudge();
	require_once plugin_dir_path( __FILE__ ) . 'admin/class-pfm-review-request.php';
	new PFM_Review_Request();
	require_once plugin_dir_path( __FILE__ ) . 'admin/class-rex-feed-validation-promo-banner.php';
	new Rex_Feed_Validation_Promo_Banner();
}
